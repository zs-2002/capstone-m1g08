...
class SupervisorInterface:
    def __init__(self):
        ...
        # Video feed placeholder with border (hidden initially)
        self.video_frame = tk.Frame(self.window, bg="black", bd=2, relief="ridge")
        self.video_label = tk.Label(self.video_frame, text="VIDEO", font=("Arial", 14), fg="white", bg="black", width=50, height=20)

        # Input fields
        tk.Label(self.window, text="Process Name:").grid(row=1, column=0)
        self.process_name = tk.Entry(self.window)
        self.process_name.grid(row=1, column=1)

        tk.Label(self.window, text="Time Required (s):").grid(row=2, column=0)
        self.time_required = tk.Entry(self.window)
        self.time_required.grid(row=2, column=1)

        # Buttons
        tk.Button(self.window, text="Start Visualization", command=self.start_visualization).grid(row=3, column=0)
        tk.Button(self.window, text="Add Step", command=self.add_step).grid(row=3, column=1)
        tk.Button(self.window, text="Stop Visualization", command=self.stop_visualization).grid(row=4, column=0)
        tk.Button(self.window, text="Save Steps", command=self.save_steps).grid(row=4, column=1)

        # List processes and allow user to type a process
        tk.Label(self.window, text="Available Processes:").grid(row=5, column=0, columnspan=2)
        self.process_list_display = tk.Text(self.window, height=5, width=60, state='disabled')
        self.process_list_display.grid(row=6, column=0, columnspan=2)

        tk.Label(self.window, text="Type Current Process:").grid(row=7, column=0)
        self.current_process_entry = tk.Entry(self.window)
        self.current_process_entry.grid(row=7, column=1)

        tk.Button(self.window, text="Start Current Process", command=self.start_operator_interface).grid(row=8, column=1)

        self.step_display = tk.Text(self.window, height=10, width=60)
        self.step_display.grid(row=9, column=0, columnspan=2)
        ...
        if not self.is_visualizing:
            if not self.process_name_set:
                ...
                if process:
                    ...
                    tk.Label(self.window, text="Please enter a process name before starting!", fg="red").grid(row=10, column=0, columnspan=2)
                    return
    ...
    def add_step(self):
        if not self.is_visualizing:
            tk.Label(self.window, text="Start visualization before adding steps", fg="red").grid(row=10, column=0, columnspan=2)
            return
        ...
        if time.isdigit():
            ...
            tk.Label(self.window, text="Invalid input. Please try again!", fg="red").grid(row=10, column=0, columnspan=2)
    ...
    def stop_visualization(self):
        if self.is_visualizing:
            ...
            tk.Label(self.window, text="Visualization is not running!", fg="red").grid(row=10, column=0, columnspan=2)

    def save_steps(self):
        if not self.steps:
            tk.Label(self.window, text="No steps to save!", fg="red").grid(row=10, column=0, columnspan=2)
            return

        # Convert the steps to DataFrame
        df = pd.DataFrame(self.steps)
        df.to_csv("assembly_steps.csv", index=False)
        tk.Label(self.window, text="Steps Saved!", fg="green").grid(row=10, column=0, columnspan=2)
    ...
    def start_operator_interface(self):
        ...
        if selected_process not in available_processes:
            tk.Label(self.window, text="Invalid process selected!", fg="red").grid(row=11, column=0, columnspan=2)
            return
