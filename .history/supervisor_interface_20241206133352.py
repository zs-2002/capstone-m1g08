    ...
    def start_visualization(self):
        if not self.is_visualizing:
            if not self.process_name_set:
                ...
                if process:
                    self.step_display.insert(tk.END, f"Process Name: {process}\n")
                    self.process_name_set = True
                    self.process_name.config(state='disabled')  # Disable input for process name
                else:
                    tk.Label(self.window, text="Please enter a process name before starting!", fg="red").grid(row=5, column=0, columnspan=2)
                    return
